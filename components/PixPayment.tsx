...
export default function PixPayment({ preferenceId, onSuccess, onCancel }: PixPaymentProps) {
  ...
  // Função para quando o pagamento for confirmado
  ...
  if (paymentConfirmed) {
    return (
      <div className="text-center p-8">
        ...
        <h3 className="text-2xl font-bold text-theme-primary mb-3">Pagamento Confirmado!</h3>
    ...
    <div className="max-w-md mx-auto p-8 bg-theme-card rounded-2xl shadow-xl border border-theme-border-primary">
      ...
      <div className="text-center mb-8">
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 p-4 rounded-xl border border-blue-200 dark:border-blue-800">
          <div className="flex items-center justify-center space-x-3 text-blue-600 dark:text-blue-400 mb-2">
            <Clock className="w-6 h-6" />
            <span className="font-medium">PIX válido por 15 minutos</span>
          </div>
          <p className="text-sm text-blue-700 dark:text-blue-300">
            Valor: R$ {(pixData.value / 100).toFixed(2).replace('.', ',')}
          </p>
        </div>
      </div>
